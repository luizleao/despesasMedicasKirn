...
class PDF extends FPDF {
    public $aColunas = [60, 60, 30, 75, 12, 10, 15, 15];
    public $oSicasFatura;
    ...
    function Header() {
        ...
        $this->Ln();
        $this->Cell(282, 6, utf8_decode('Fatura do Credenciado'), 0, 0, 'C', false);
        $this->Ln();
        $this->Cell(110, 6, utf8_decode('Credenciado:'), 0, 0, 'R', false);
        $this->SetFont('Arial', 'B', 6);
        $this->Cell(50, 6, utf8_decode($this->oSicasFatura->oSicasCredenciado->nm_credenciado), 0, 0, 'L', false);
        $this->SetFont('Arial', '', 6);
        ...
        $header = ['Servidor', 'Beneficiário', 'Guia', 'Procedimento', 'Data', 'Qtde.', 'Vl. Unitario', 'Subtotal'];
...
foreach($aDespesa as $oDespesa) {
    ...
    $oPDF->Cell($oPDF->aColunas[3], 4, utf8_decode($oProcedimentoAutorizado->oSicasProcedimento->nm_procedimento),                                            1, 0, 'L', false);
    $oPDF->Cell($oPDF->aColunas[4], 4, utf8_decode(Util::formataDataHoraBancoForm($oProcedimentoAutorizado->oSicasEncaminhamento->dt_encaminhamento, false)), 1, 0, 'L', false);
    $oPDF->Cell($oPDF->aColunas[5], 4, utf8_decode($oDespesa->qtd_servico_realizado),                                                         1, 0, 'L', false);
    $oPDF->Cell($oPDF->aColunas[6], 4, utf8_decode(Util::formataMoeda($oDespesa->val_servico_realizado)), 					                                  1, 0, 'R', false);
    $oPDF->Cell($oPDF->aColunas[7], 4, utf8_decode(Util::formataMoeda($oDespesa->qtd_servico_realizado * $oDespesa->val_servico_realizado)), 		1, 0, 'R', false);
...
$oPDF->Cell($oPDF->aColunas[1]+$oPDF->aColunas[2]+$oPDF->aColunas[3]+$oPDF->aColunas[4]+$oPDF->aColunas[5]+$oPDF->aColunas[6]+$oPDF->aColunas[7], 4, utf8_decode(Util::formataMoeda($totalFatura)), 1, 0, 'R', false);
$oPDF->Ln();
